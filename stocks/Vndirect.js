...
const URL_STOCK_CODE_LIST = "https://price-as01.vndirect.com.vn/priceservice/secinfo/snapshot/q=floorCode:10,02,03";
...
function getLastStockData(stockCode) {
    ...
    return fetch(URL_STOCK_HISTORY + stockCode + "&from=" + strFrom + "&to=" + strTo, {
            "headers": {
                "accept": "application/json, text/plain, */*",
            }
        })
        ...
        .then(data => {
            ...
            if (dataSize === 0) {
                return Promise.reject({ errMsg: "[getLastStockData] have no record to pull. code=" + stockCode });
            }
...
function initDataForTheFirstTime() {
    getCodeList().then(codeList => {
        var allPromise = Promise.map(codeList, getStockHistoryAll,{concurrency:4});
